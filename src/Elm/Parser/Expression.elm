...
literalExpression =
    Tokens.singleOrTripleQuotedStringLiteral
        |> Combine.fromCoreMap (\( stringLiteralType, str ) -> StringLiteral stringLiteralType str)
    ...
    Core.map
        (\( startRow, startColumn ) ->
            \firstArg ->
                \args ->
                    \((Node { end } _) as expr) ->
                        { firstArg = firstArg, restOfArgs = args, expression = expr }
                            |> LambdaExpression
        ...
        |. Tokens.backSlash
        |> Combine.fromCoreIgnore Layout.maybeLayout
        |> Combine.keep DestructurePatterns.destructurePattern
        |> Combine.keep
            (Combine.many
                (Layout.maybeLayout |> Combine.continueWith DestructurePatterns.destructurePattern)
            )
        |> Combine.ignore Layout.maybeLayout
...
numberExpression =
    Elm.Parser.Numbers.forgivingNumber FloatLiteral IntegerLiteral HexLiteral
        |> Combine.fromCore
